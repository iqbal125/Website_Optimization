...
function sizeSwitcher (size) {
  switch(size) {
    ...
    //randomPizzaContainer container class stored in this variable for better performance of the for loop
    var randomPizzas = document.querySelectorAll(".randomPizzaContainer")
    for (var i = 0; i < randomPizzas.length; i++) {
      //iterates through the class and changes the pizza size based on user selection
...
function updatePositions() {
  ...
  //getElementsByClassName used instead of querySelector for faster rendering
  var items = document.getElementsByClassName('mover');

  //scroll position is stored in this variable
  var scroll = document.body.scrollTop

  //The results of the smaller loop are stored here
  var PhaseArr = []

  //Hard calculations are done in the smaller loop for better response
  for(var i = 0; i < 5; i++) {
    PhaseArr.push(Math.sin((scroll / 1250) + (i % 5)));
  }

  //animates the pizzas as the user scrolls by a certain number of pixels
  for(var j = 0; j < items.length; j++) {
    var phase = PhaseArr[j % 5] * 100
